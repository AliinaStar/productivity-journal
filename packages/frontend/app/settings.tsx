...
export default function SettingsScreen() {
  ...
  async function handleSave() {
    if (!name.trim()) { setError("Введи ім'я"); return; }
    setSaving(true);
    setError(null);
    try {
      const userId = await syncMeta.getUserRemoteId();
  ...
  return (
    <ScrollView style={s.scroll} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
      <Text style={s.title}>Налаштування</Text>

      <Text style={s.label}>Ім'я</Text>
      <TextInput
        style={s.input}
        placeholder="Ім'я"
      ...
      <TouchableOpacity style={s.btn} onPress={handleSave} disabled={saving} activeOpacity={0.8}>
        {saving ? <ActivityIndicator color="#fff" /> : <Text style={s.btnText}>Зберегти</Text>}
      </TouchableOpacity>
